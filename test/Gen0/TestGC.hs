...
testOneEnv = testCase "one env" $ do
  assertEqual "addr" (Addr 1) addr
  assertEqual "store orig"
    (Map.fromList [(Addr 1, StoreVal StoreWhite ValVoid)])
    (stateSpace state)
  assertEqual "store new"
    (Map.fromList [(Addr 1, StoreVal StoreWhite ValVoid)])
    (stateSpace state')
  assertEqual "store old"
    (Map.fromList [(Addr 1, StoreForward (Addr 1))])
    (stateSpace stateEvac)
  where
    state = State exp env store Halt
    (stateEvac, state') = garbageCollect state
    exp = ExpAtomic AExpFalse
    env = Map.fromList [(Var "x", addr)]
    (store, addr) = storeAlloc storeEmpty ValVoid StoreWhite

testOneCont :: Test
testOneCont = testCase "one cont" $ do
  assertEqual "addr" (Addr 1) addr
  assertEqual "store orig"
    (Map.fromList [(Addr 1, StoreVal StoreWhite val)])
    (stateSpace state)
  assertEqual "store new"
    (Map.fromList [(Addr 1, StoreVal StoreWhite val)])
    (stateSpace state')
  ...
  where
    val = ValInt 6
    state = State exp envEmpty store cont
    (stateEvac, state') = garbageCollect state
    exp = ExpAtomic AExpFalse
    env = Map.fromList [(Var "x", addr)]
    cont = Cont (Var "y") (ExpAtomic $ AExpInt 2) env Halt
    (store, addr) = storeAlloc storeEmpty val StoreWhite
